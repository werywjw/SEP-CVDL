...
% TODO
% citations for the entire paper in introduction
\paragraph{Motivation.}
The goal of our research and development was to build a robust and interpretable model for facial emotion recognitions. 
...
% DONE (<= 4 bullet points usually very shot without citations)
Our main contributions can be summarized as follows. 

\begin{itemize}
  \item We collect, preprocess, 
  and evaluate the training and testing data (both image and video) thoroughly from various public databases. 
...
% TODO: FER image/video
% See e.g. for introduction writing: 
...
which corrsponds to different classes by exponentiating each logit and then normalizing these values by the sum of all exponentiated logits. 
Mathemathically, for a given logit $z_i$ among $n$ classes, 
...
the performance of all our models on FER2013 is worse than on on RAF-DB. 
...
which is 7 times less than the VGG16 and half than ResNet34, 
...
one reason is that the images on RAF-DB is already aligned and cropped, 
...
We further continue to simply apply merely normalization to detact more representative features with respect to different emotions.
\citet{ZeilerF14,li_reliable_2017,VermaMRMV23} engage similar investigations. 

We run extensive experiments to determine the number of the convolution blocks for a better trade-off between the number of parameters and the performance. 
...
However, six convolutional blocks with 17 layers do not necessarily result in a better performance, 
...
as such regularization techniques can prevent overfitting by by randomly dropping units 
(along with their connections) from the neural networks during training. 
Also the batch normalization can help the model to stabilize the learning process and reduce the number of training epochs, 
leading to a faster learn progress and thus improving the generalization ability of the model. 
...
For augmentation we determined at the very begining. 
