...
we acquired the image databases such as 
\textit{Real-world Affective Faces Database} (RAF-DB~\cite{li_reliable_2017,li2019reliable}), 
\textit{Facial Expression Recognition} 2013 (FER2013~\cite{BarsoumZCZ16}), 
\textit{FER AffectNet Database} (FER AffectNet~\cite{Mollah2019ANet}), 
\textit{Extended Cohn-Kanade Dataset Plus} (CK+~\cite{LuceyCKSAM10}), 
and \textit{Taiwanese Facial Expression Image Database} (TFEID~\cite{tfeid,LiGL22}), 
as well as the video database 
\textit{Denver Intensity of Spontaneous Facial Action Database} (DISFA~\cite{mavadati2013disfa}) 
from public institutions and kaggle~\cite{kaggle_rafdb,kagaff}. 

\paragraph{Image Preprocessing.}
Build upon these databases, 
we exclusively analyze human faces representing 6 emotions. 
That is, 
we first generalize a folder structure annotating the labels of happiness (0), surprise (1), sadness (2), 
anger (3), disgust (4), and fear (5). 
Afterward, we append the emotion labels to the name of each matching image. 
In order to later efficiently pass the images to the model, 
we also create a CSV file to store all the image and their corresponding labels. 
We use a custom dataset by augmentation to increase the variety, 
which includes \texttt{RandomHorizontalFlip}, \texttt{RandomRotation}, \texttt{RandomCrop}, and \texttt{RandomErasing}. 
...
\% to compare the performance of our \textsc{GiMeFive} with other state-of-the-art models. 
