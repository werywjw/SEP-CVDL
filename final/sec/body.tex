...
\Cref{fig:modeldetail} illustrates the empirical results of our current best model. 
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    \node [box, dashed, right=of mlmodel, fill=cvprblue!30] (interpretability) {Interpretability};
    \node [box, dashed, right=of interpretability, fill=LightPurple!30] (inspection) {Human Inspection};
    \node [box, dashed, below=of inspection] (predictions) {Verified Predictions};

    \path [line] (data) -- (mlmodel);
    \path [line] (mlmodel) -- (preds);
    \path [line] (mlmodel) -- (interpretability);
    \path [line] (interpretability) -- (inspection);
    \path [line] (inspection) |- node[above left] {Data Improvement} (data);
    \path [line] (inspection) -- node[right] {Model Improvement}  (predictions);

  \end{tikzpicture}
  }
  \caption{Overview of the explainable AI compared with standardized ML evaluation process} 
  \label{fig:xai}
\end{figure}
...
\begin{table*}[ht]
  ...
  \begin{tabular}{@{}lccc@{}}
    \toprule
    Dataset & \#Images in training set & \#Images in testing set & \# Images in validation set  \\
    \midrule
    Real-world Affective Faces Database (RAF-DB~\cite{li_reliable_2017,li2019reliable}) & 9747 & 2388 & 600 \\
    Facial Expression Recognition Plus (FER+~\cite{BarsoumZCZ16}) & 23743 & 5945 & 600  \\
    Extended Cohn-Kanade Dataset Plus (CK+~\cite{LuceyCKSAM10}) &&-& 600 \\
    Taiwanese Facial Expression Image Database (TFEID~\cite{tfeid,LiGL22}) &&-& 600 \\
    \bottomrule
  \end{tabular}
  }
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[node distance=1.7cm]
    ...
    \node (fc3) [fc, below of=dropout2] {FC3 $6$};
...
\begin{table*}[ht]
  ...
  \begin{tabular}{@{}llccccr@{}}
    ...
    \cline{4-6}
    &&& \textsc{Training} & \textsc{Testing} & \textsc{Validation}  &  \\
    \midrule
    ...
    &CNN (4 layers) & +BN-SE & 96.6 & 80.6 & 66.8 & 2606086 \\ 
    &CNN (5 layers) & -BN-SE & 96.3 & 76.9 & 60.6 & 10474118 \\
    &CNN (5 layers) & +BN+SE & 98.4 & 81.7 & 71.1 & 10478598 \\
    &CNN (5 layers) & +BN-SE & 98.6 & \textbf{83.1} & \textbf{72.1} & 10478086 \\
    &CNN (6 layers) & +BN-SE & 97.5 & 82.5 & 70.0 & 41950726 \\ 
    \midrule
    \multirow{3}{*}{FER+~\cite{BarsoumZCZ16}} & CNN (4 layers) & +BN-SE & 86.6 & 64.1 & 40.2 & 2606086 \\
    &CNN (5 layers) & +BN-SE & 89.6 & 65.6 & 40.7 & 10478086 \\
    &CNN (6 layers) & +BN-SE & \textbf{96.0} & \textbf{65.5} & \textbf{41.6} & 41950726 \\
    \bottomrule
  \end{tabular}
  % }
...
\Cref{fig:model} shows the test result aggregated from the database RAF-DB~\cite{kaggle_rafdb}.
