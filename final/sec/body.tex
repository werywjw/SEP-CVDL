...
Studies of the kaleidoscope of human emotions has been a central pursuit in psychology for centuries. 
...
Surprise (see \cref{fig:xai2}) implys activation around the eyes and eyebrows, which are typically raised in a look in human faces. 
...
an image pyramid, and an sliding window detection scheme~\cite{dlib_site}. 
...
brightness, color channels, or perspectives on RAF-DB dataset. 
...
forcing to make more nuanced and robust predictions. 
...
As a result, 
The learning rate for all our \textsc{GiMeFive} models is set to $0.001$ with the stochatic gradient descent optimizer. 
...
We acknowledge that our work has several constrains in terms of the accuracies of the models on the leadboard. 
