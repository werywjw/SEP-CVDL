...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    \begin{axis}[
        ...
        ylabel={Accuracies (\%)}, 
        symbolic x coords={VGG16, ResNet18,ResNet50,Basline,GiMeFive},
        xtick=data,
    ...
    % \addplot coordinates {(VGG16,90) (ResNet18,85) (ResNet50,88) (Basline,92) (GiMeFive,87)}; % Change here, fake numbers now
    \addplot[fill=red] coordinates {(VGG16,90)};
    \addplot[fill=red!50] coordinates {(VGG16,88)};
    \addlegendentry{VGG16 Test}
    \addlegendentry{VGG16 Validation}

    \addplot[fill=blue] coordinates {(ResNet18,85)};
    \addplot[fill=blue!50] coordinates {(ResNet18,82)};
    \addlegendentry{ResNet18 Test}
    \addlegendentry{ResNet18 Validation}

    \addplot[fill=green] coordinates {(ResNet50,88)};
    \addplot[fill=green!50] coordinates {(ResNet50,85)};
    \addlegendentry{ResNet50 Test}
    \addlegendentry{ResNet50 Validation}

    \addplot[fill=orange] coordinates {(Basline,92)};
    \addplot[fill=orange!50] coordinates {(Basline,90)};
    \addlegendentry{Basline Test}
    \addlegendentry{Basline Validation}

    \addplot[fill=purple] coordinates {(GiMeFive,87)};
    \addplot[fill=purple!50] coordinates {(GiMeFive,85)};
    \addlegendentry{GiMeFive Test}
    \addlegendentry{GiMeFive Validation}
    
    % \legend{Models}
    \end{axis}
  \end{tikzpicture}
  \caption{Accuracies of our \textsc{GiMeFive} compared to other state-of-the-art models} 
  \label{fig:acc}
\end{figure}
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lcccccc@{}}
      \toprule
      \textsc{Dataset} & \#happiness & \#surprise & \#sadness & \#anger & \#disgust & \#fear \\
      \midrule
      RAF-DB (Train) & 4772 & 1290 & 1982 & 705 & 717 & 281 \\
      FER2013 (Train) & 7215 & 3171 & 4830 & 3994 & 436 & 4097 \\
      FER AffectNet (Train) & 3091 & 4039 & 5044 & 3218 & 2477 & 3176 \\
      CK+ (Train) & 69 & 83 & 28 & 45 & 59 & 25 \\
      TFEID (Train) & 40 & 36 & 39 & 34 & 40 & 40 \\
      \midrule
      \midrule
