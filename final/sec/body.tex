...
To initiate the project, we gathered image databases representing different types of emotional expressions. 
...
\Cref{tab:data} gives detailed data statistics of each training, test, and validation set on emotion classes.
...
Our result and analysis are given in \Cref{sec:optim:aug}. 
...
The Denver Intensity of Spontaneous Facial Action (DISFA~\cite{MavadatiMBTC13}) dataset comprises two sets of videos, differentiated 
by capturing various camera angles. Each set consists of 27 videos, with each video comprising 4844 frames, resulting 
in 130,788 images for each camera angle and a total of 261,576 images across both angles.
For testing our model, we established standards for the extracted frames from videos or live webcam streams. 
Each frame, extracted for testing, undergoes scanning for the region of interest, the face(s). To achieve this, 
we implemented the (OpenCV) \texttt{CascadeClassifier}~\cite{casc_class}.
OpenCV, a comprehensive computer vision library, incorporates the Viola-Jones algorithm to power Haar Cascade Classifiers 
for face detection. The Viola-Jones algorithm, introduced by Paul Viola and Michael Jones, revolutionized real-time object 
detection~\cite{990517}. 

As part of the video preprocessing, we run every frame through the Haar Cascade Classifiers to detect our region of interest, 
and crop the rectangled region of the face.
...
Every cropped image is evaluated through our model, so that the classifications can be used for further interpretation. 

We implement using the libraries such as OpenCV~\footnote{~\url{https://opencv.org}}. %% Für Cascade
...
we add the ResNet18 with the residual block to ease the training process. 
...
\begin{table*}[ht]
  ...
  The confusion matrix of the test set on the RAF-DB is given in \Cref{fig:mattest}, 
  while the result of validation test is shown in \Cref{fig:matval}.} 
  \label{tab:model}
\end{table*}
...
\paragraph{Classified emotion (Plot something).} % TODO Leah
The GiMeFive classifications of every frame are plotted in percentace next to the saliency map (Heatmap), to the right hand side.
On top of the saliency map the main emotion, extracted every fifth frame, is written, so it is easier for our eyes to adapt.

\paragraph{Dlib Landmarks.} % TODO Leah
To plot Landmarks on each video frame, we implemented the dlib face detector % \texttt{shape_predictor_68_face_landmarks.dat}, 
...
With this pertained model, 68 numbered Landmarks assist emotion analysis, and facial expression tracking, to understand and to interpret facial features.

We implement using the libraries such as OpenCV~\footnote{~\url{http://dlib.net/}}. % Für dlib

% image of GiMeFive_Video??
