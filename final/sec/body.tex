...
% introduction from Leah
% See introduction writing e.g., 
...
while \citet{PhamVT20} presented a novel masking idea to boost the performance on FER2013 and private VEMO dataset. 
...
Understanding the visual world through machines has been a driving force in computer vision for decades (Juneja et al., 2013; Mahendran & Vedaldi, 2016; Parikh & Zitnick, 2011; Singh et al., 2012; Sudderth et al., 2005). Early attempts at peering into the "black box" of recognition models included directly visualizing filter responses (Zeiler & Fergus, 2014), reconstructing inputs from network layers (Zeiler & Taylor, 2011), and even crafting inputs to activate specific neurons (Nguyen et al., 2015).
Recently, techniques like Class Activation Mapping (CAM) by Zhou et al. (2016) and Gradient-weighted CAM (Grad-CAM) by Selvaraju et al. (2017) have pushed the boundaries. While CAM leverages global pooling to pinpoint informative regions, Grad-CAM offers a more general framework for visualizing any convolutional filter, revealing insights hidden within neural networks.
The quest for meaningful representations in face recognition goes back even further (Chen et al., 2002; Learned-Miller, 2016; Toole et al., 2018). Traditional approaches often prioritized accuracy over interpretability, building representations from either facial parts (Li et al., 2013; Cao et al., 2010) or facial attributes (Kumar et al., 2009). While these methods achieved progress, they lacked transparency in how they captured and utilized facial information.
The field of interpretable representation learning aims to bridge this gap. By delving deeper into the inner workings of models, we can understand not just what, but also why and how they make decisions. This newfound transparency holds immense potential for applications where trust and explainability are critical, such as healthcare, law enforcement, and autonomous vehicles.
...
The right side of each sub-figure is result of the overlay Grad-CAM. 
For instance, 
