...
\subsection{Explainable AI \& Grad-CAM} % aggregate
\label{sec:optim:cam}
...
Besides proposing a method to visualize the discriminative regions of a classification-trained CNN, the authors also use this method to localize objects without providing the model with any bounding box annotations. % ??? which authors, if others please cite, if us please say we => the authors do not make sense here anymore.. the authors of the CAM-Paper.
The model just learns the classification task with class labels and is then able to localize the object of a specific class in an image. 
